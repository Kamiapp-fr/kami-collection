...
// TODO: Remove eslint-disable and fix the error.
// import lib
 ...
 */
class KamiFlash extends KamiComponent<KamiFlashState> {
  /**
     * @static
  ...
  declare protected props: KamiFlashState;
  ...
  public setProperties(): void {
    this.inLoad = true;
    const type: any = this.getAttribute('type') || 'OK';
    const position: string = this.getAttribute('position') || 'BOTTOM';
  ...
  public unStackFlash(): void {
    KamiFlash.stackedFlash[this.position].forEach((flash: this) => {
      ...
      if (flash.index > this.index) {
        ...
        flash.stackedPosition -= KamiFlash.ofsetPosition;
  ...
  public static createFlash(
    ...
    const flash = document.createElement(tagName);
