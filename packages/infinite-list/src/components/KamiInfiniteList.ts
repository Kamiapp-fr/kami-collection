...
class KamiInfiniteList extends KamiComponent {
  ...
  /**
     * @property {
     *  CustomEvent<IClickElementEvent>
     * } clickElementEvent - event when an element of the list is clicked.
     */
  ...
  public setProperties(): void {
    const datasource: string | null = this.getAttribute('datasource');
    const delegate: string | null = this.getAttribute('delegate');
    ...
    this.container.addEventListener('scroll', () => {
      if (
        ...
        if (!this.inLoad && !this.end) {
          // increment the page
          this.props.query[this.props.pageQuery] += 1;
        ...
        if (Array.isArray(json)) {
          ...
          for (this.data in json) {
            ...
            if (data instanceof Object && !Array.isArray(data)) {
              const component = this.component.cloneNode();

              this.componentAttributes.forEach((atts) => {
                const dataProvide = this.convertData(
                  data,
  ...
  /**
     ...
     */
  public convertData(obj: any, path: any, separator = '.'): any {
